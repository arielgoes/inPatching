...
FRR_VERSION="FRR_round_robin_stop-w-wait_3exec"
...
for threshold in $TIME_OUTS; do
    for ((j=0; j<${#B[@]}-1; j++)); do
        ...
        if [[ ${B[j]} == 's5' && ${B[j+1]} == 's1' ]]; then
            STD_VAR=$(cat results/scapy_socket/convergence_${threshold}us/data_exec3\(${FRR_VERSION}\)/aux/STATS_${B[j+1]}-${B[j]}_${threshold}us.txt | grep -P "(?<=stddev_y).*\d+" | cut -f2)
            ...
            STD_VAR=$(cat results/scapy_socket/convergence_${threshold}us/data_exec3\(${FRR_VERSION}\)/aux/STATS_${B[j]}-${B[j+1]}_${threshold}us.txt | grep -P "(?<=stddev_y).*\d+" | cut -f2)

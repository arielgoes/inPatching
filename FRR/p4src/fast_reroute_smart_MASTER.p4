...
control MyIngress(inout headers hdr,
    ...
    apply {
        if (hdr.ipv4.isValid()){
            ...
            if(swId == depotId && hdr.pathHops.has_visited_depot == (bit<8>)0){
            ...
            if(swId == depotId && sw_overlap_var > 0 && hdr.pathHops.path_id == (bit<32>)0 && hdr.pathHops.has_visited_depot == (bit<8>)0){
            ...
            //FRR control (all the decisions are made at the depot/starting node)
                ...
                if(hdr.pathHops.path_id == (bit<32>)0 && (hdr.pathHops.pkt_id == token_pkt_id || sw_overlap_var > 0)){ //first flow...
                    ...
                    if(path_id_pointer_var == 0){ //if the pointer is at the first position (index 0), ...
                        ...
                        path_id_pointer_reg.write(hdr.pathHops.path_id, 1); //...reset it to 1 - i.e., first alternative hop
                        path_id_pointer_reg.read(path_id_pointer_var, hdr.pathHops.path_id); //... and read again for the updated pointer value
                    }
            ...
            /*else if(swId == depotId && hdr.pathHops.is_tracker > 0 && hdr.pathHops.has_visited_depot > (bit<8>)0){ //special case (is_tracker) probe
            ...
            if(swId == depotId && (bit<32>)hdr.pathHops.numHop >= meta.lenPrimaryPathSize && hdr.pathHops.has_visited_depot > (bit<8>)0){
                read_depot_port();

...
for threshold in $TIME_OUTS; do
    for ((j=0; j<${#B[@]}-1; j++)); do
        ...
        if [[ ${B[j]} == 's5' && ${B[j+1]} == 's1' ]]; then
            STD_VAR=$(cat results/scapy_socket/convergence_${threshold}us/data_exec3\(FRR_round_robin\)/aux/STATS_${B[j+1]}-${B[j]}_${threshold}us.txt | grep -P "(?<=stddev_y).*\d+" | cut -f2)
            echo "${B[j+1]}-${B[j]} ${STD_VAR}" >> results/scapy_socket/convergence_${threshold}us/data_exec${VERSION_EXEC}\(FRR_round_robin\)/STATS_${threshold}us_${LINES_OR_OTHER}_AVG_${SINGLE_OR_TWO_FLOWS}.txt
        else
            STD_VAR=$(cat results/scapy_socket/convergence_${threshold}us/data_exec3\(FRR_round_robin\)/aux/STATS_${B[j]}-${B[j+1]}_${threshold}us.txt | grep -P "(?<=stddev_y).*\d+" | cut -f2)
            echo "${B[j]}-${B[j+1]} ${STD_VAR}" >> results/scapy_socket/convergence_${threshold}us/data_exec${VERSION_EXEC}\(FRR_round_robin\)/STATS_${threshold}us_${LINES_OR_OTHER}_AVG_${SINGLE_OR_TWO_FLOWS}.txt
        fi
    done
done
com
